...
all_models_for_plot <- bind_rows("me1" = me_plot1, 
  ...
  filter(coef_name %in% c("beta.0", "beta.age2", "beta.age3", "beta.bmi")) %>% 
  tidyr::separate(coef_name, c("sub_model", "coef_name")) %>% 
  mutate(coef_pretty = paste0(sub_model, "[", coef_name, "]")) %>% 
  mutate(model = recode(model, "me1" = "Simple imputation model", "me2" = "More complex imputation model", "naive" = "Complete case analysis", "inla_mcmc" = "Goméz-Rubio & Rue (2018)")) %>% 
  mutate(model = fct_relevel(model, levels = c("More complex imputation model", "Simple imputation model", "Goméz-Rubio & Rue (2018)", "Complete case analysis"))) %>% 
...
ggplot(all_models_for_plot, aes(y = model)) +
  ...
  scale_color_manual(values = darken(ggthemes::canva_palettes$"Subtle and versatile", 0.4)) +
