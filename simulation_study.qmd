...
simulation_results <- readRDS("results/simulation_results.rds") %>% 
  ...
  mutate(model = fct_relevel(model, levels = c("naive", "true", "ME"))) %>% 
...
ggplot(simulation_results, aes(x = 1, y = value, groups = model)) +
  ...
  stat_summary(aes(color = stage(model, after_scale = darken(color, 0.4))),
               geom = "linerange",
  ...
  stat_summary(data = dplyr::filter(simulation_results, variable == "beta[0]"),
               aes(x = 1, y = value + 0.1*value, label = model,
                   color = stage(model, after_scale = darken(color, 0.4))),
               geom = "text", fun = max,
               position = position_dodge(width = 0.75),
               size = 3, fontface = "bold", family = f1) +
  ...
  scale_color_manual(values = ggthemes::canva_palettes$"Subtle and versatile"[c(4, 1, 3)]) +
  ylab("Posterior mean") +
  theme_titles
