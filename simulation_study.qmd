...
#| purl: false
#| include: false
knitr::purl(input = "simulation_study.qmd", 
            output = "R/simulation_study.R")
```
...
true_values <- data.frame(variable = c("beta[0]", "beta[x]", "beta[z]"), 
                          true_value = c(1, 2, 2))

simulation_results <- readRDS("results/simulation_results.rds") %>% 
  mutate(variable = paste0("beta[", variable, "]")) %>% 
  mutate(model = fct_relevel(model, levels = c("true", "ME", "naive"))) %>% 
  mutate(model = recode(model, "ME" = "ME model", "true" = "Best case model", 
                        "naive" = "Complete case\nmodel")) %>% 
  group_by(variable, model) %>% 
  mutate(mean = mean(value)) %>% 
  full_join(true_values, by = "variable") 
...
theme_titles <- theme_minimal(base_size = 18, base_family = f1) + 
  ...
  axis.line.x.top = element_line(size = 1, color = "grey65"),
...
ggplot(simulation_results, aes(x = 1, y = value, groups = model)) +
  ...
  geom_point(aes(color = stage(model, after_scale = lighten(color, 0.4))), 
             alpha = 0.5, size = 1,
             position = position_jitterdodge(jitter.width = 0.4, seed = 1)) +
  # Error bar for confidence interval
  stat_summary(aes(color = model),
               geom = "linerange",
               fun.min = function(z) {mean(z) - 1.96*sd(z)},
               fun.max = function(z) {mean(z) + 1.96*sd(z)},
  ...
  stat_summary(data = dplyr::filter(simulation_results, variable == "beta[0]"),
               aes(x = 1, y = value + 0.15*value, label = model, color = model),
               geom = "text", fun = max,
               position = position_dodge(width = 0.75),
               size = 4.3, fontface = "bold", family = f1) +
  ...
  # Make second x-axis so we get grey line on top
  scale_x_continuous(sec.axis = sec_axis(trans = ~.)) +
  # Breakpoints for y-axis
